...
class mViT(nn.Module):
    def __init__(self, in_channels, n_query_channels=128, patch_size=16, dim_out=256,
                 embedding_dim=128, num_heads=4, norm='linear', max_seq_len=500, do_concat_to_patch_feats=False):
    ...
    def forward(self, x, concat_to_patch_feats=None):
        # n, c, h, w = x.size()
        
        tgt = self.patch_transformer(x.clone(), concat_to_patch_feats)  # .shape = S, N, E
        x = self.conv3x3(x)
